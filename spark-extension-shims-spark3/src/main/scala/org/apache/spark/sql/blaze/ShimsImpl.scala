...
class ShimsImpl extends Shims with Logging {
  ...
  override def convertExpr(e: Expression): Option[pb.PhysicalExprNode] = {
    e match {
      ...
      case _ =>
        convertPremotePrecision(e) match {
  ...
  @enableIf(Seq("spark303", "spark234", "spark333").contains(System.getProperty("blaze.shim")))
  def convertPremotePrecision(e: Expression): Option[pb.PhysicalExprNode] = {
    e match {
      case org.apache.spark.sql.catalyst.expressions.PromotePrecision(_1) =>
        _1 match {
          case cast: Cast if cast.dataType == _1.dataType =>
            Shims.get.convertExpr(_1)
          case _ =>
            Shims.get.convertExpr(Cast(_1, _1.dataType))
        }
      case _ => None
    }
  }
  ...
  def convertPremotePrecision(e: Expression): Option[pb.PhysicalExprNode] = None
