...
pub fn blaze_call_native(
    ...
    let task_definition_raw = env.get_direct_buffer_address(task_definition)
    ...
    let task_id = task_definition.task_id.expect("Missing task_definition.task_id");
    let plan = &task_definition.plan.expect("Missing task_definition.plan");
    let execution_plan: Arc<dyn ExecutionPlan> = plan.try_into()
        .expect("Error converting to ExecutionPlan");
    info!("Creating native execution plan succeeded: task_id={:?}", task_id);
    ...
    let execution_plan = replace_shuffle_reader(execution_plan.clone(), &env, &task_id.job_id);
    ...
    let record_batch_stream = sync_tokio_runtime
        .block_on(execution_plan.execute(task_id.partition_id as usize))
    ...
    let record_batches: Vec<RecordBatch> = sync_tokio_runtime
        .block_on(datafusion::physical_plan::common::collect(record_batch_stream))
    ...
    for metric in execution_plan.metrics().unwrap_or_default().iter() {
        info!(" -> metrics (partition={}): {}: {}",
            metric.partition().unwrap_or_default(),
            metric.value().name(),
            metric.value())
    }
    let record_batches = record_batches.into_iter() // retain non-empty record batches
        .filter(|record_batch| record_batch.num_rows() > 0)
        .collect::<Vec<_>>();

    let consumer_class = env.find_class("java/util/function/Consumer").unwrap();
    let consumer_accept_method = env.get_method_id(consumer_class, "accept", "(Ljava/lang/Object;)V").unwrap();

    if !record_batches.is_empty() {
        info!("Result schema:");
        for field in schema.fields() {
            info!(" -> col={}, type={}, nullable={}", field.name(), field.data_type(), field.is_nullable())
        }
        ...
        let byte_buffer = env.new_direct_byte_buffer(buf_writer.get_mut()).expect("Error creating ByteBuffer");
...
fn replace_parquet_scan_object_store(plan: Arc<dyn ExecutionPlan>, env: &JNIEnv) -> Arc<dyn ExecutionPlan> {
    ...
    if let Some(parquet_exec) = plan.as_any().downcast_ref::<ParquetExec>() {
        let parquet_scan_base_config = unsafe {
            // safety: bypass visiblity of PhysicalPlanConfig
            &mut *std::mem::transmute::<*const PhysicalPlanConfig, *mut PhysicalPlanConfig>(
                parquet_exec.base_config() as *const PhysicalPlanConfig)
        };
        parquet_scan_base_config.object_store = Arc::new(HDFSSingleFileObjectStore::new(env.get_java_vm().unwrap()));
    }
...
fn replace_shuffle_reader(plan: Arc<dyn ExecutionPlan>, env: &JNIEnv, job_id: &str) -> Arc<dyn ExecutionPlan> {
    let children = plan.children().iter()
    ...
    if let Some(shuffle_reader_exec) = plan.as_any().downcast_ref::<ShuffleReaderExec>() {
        let blaze_shuffle_reader: Arc<dyn ExecutionPlan> = Arc::new(BlazeShuffleReaderExec {
