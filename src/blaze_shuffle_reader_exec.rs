...
impl ExecutionPlan for BlazeShuffleReaderExec {
    ...
    async fn execute(&self, _partition: usize) -> Result<SendableRecordBatchStream> {
        let jni_get_buffers = || -> JniResult<JObject<'static>> {
            ...
            info!("Shuffle reader FetchIterator resource key: {}", &resource_key);
            let buffers = env.call_static_method_unchecked(
                JavaClasses::get().cJniBridge.class.clone(),
                JavaClasses::get().cJniBridge.method_get_resource.clone(),
                JavaClasses::get().cJniBridge.method_get_resource_ret.clone(),
        ...
        let buffers = jni_get_buffers().map_err(|_|
            Util::wrap_default_data_fusion_io_error("JNI error: BlazeShuffleReaderExec.execute.jni_get_buffers"))?;
...
impl ShuffleReaderStream {
    pub fn new(jvm: JavaVM, schema: SchemaRef, buffers: JObject<'static>) -> ShuffleReaderStream {
        ShuffleReaderStream {
            jvm,
            schema,
            buffers,
    ...
    fn next_seekable_byte_channel(&mut self) -> Result<bool> {
        if self.seekable_byte_channels_pos == self.seekable_byte_channels_len && !self.next_seekable_byte_channels()? {
    ...
    fn next_seekable_byte_channels(&mut self) -> Result<bool> {
        ...
        return jni_next_seekable_byte_channels().map_err(|_|
            Util::wrap_default_data_fusion_io_error("JNI error: ShuffleReaderStream.jni_next_seekable_byte_channels"));
    }

    fn buffers_has_next(&self) -> Result<bool> {
        let jni_buffers_has_next = || -> JniResult<bool> {
            let env: &JNIEnv = &Util::jni_env_clone(&self.jvm.attach_current_thread_permanently()?);
            return Ok(env.call_method_unchecked(
                self.buffers,
                JavaClasses::get().cScalaIterator.method_has_next.clone(),
                JavaClasses::get().cScalaIterator.method_has_next_ret.clone(),
                &[],
            )?.z()?);
        };
        return jni_buffers_has_next().map_err(|_|
            Util::wrap_default_data_fusion_io_error("JNI error: ShuffleReaderStream.jni_buffers_has_next"));
    }
}
...
impl Read for SeekableByteChannelReader {
    fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
        let mut jni_read = || -> JniResult<usize> {
            ...
            return Ok(env.call_method_unchecked(
                self.1,
                JavaClasses::get().cJavaNioSeekableByteChannel.method_read.clone(),
                JavaClasses::get().cJavaNioSeekableByteChannel.method_read_ret.clone(),
                &[JValue::Object(env.new_direct_byte_buffer(buf)?.into())],
            )?.i()? as usize);
        };
        return jni_read().map_err(|_|
            std::io::Error::new(std::io::ErrorKind::Other, "JNI error: SeekableByteChannelReader.jni_read"));
    }
}
impl Seek for SeekableByteChannelReader {
    fn seek(&mut self, pos: SeekFrom) -> std::io::Result<u64> {
            ...
            match pos {
                SeekFrom::Start(position) => {
                    env.call_method_unchecked(
                        self.1,
                        JavaClasses::get().cJavaNioSeekableByteChannel.method_position_set.clone(),
                ...
                SeekFrom::End(offset) => {
                    let size = env.call_method_unchecked(
                        self.1,
                        JavaClasses::get().cJavaNioSeekableByteChannel.method_size.clone(),
                        JavaClasses::get().cJavaNioSeekableByteChannel.method_size_ret.clone(),
                        &[],
                    )?.j()? as u64;
        ...
        return jni_seek().map_err(|_|
            std::io::Error::new(std::io::ErrorKind::Other, "JNI error: SeekableByteChannelReader.jni_seek"));
    }
}
