...
impl ObjectStore for HDFSSingleFileObjectStore {
    async fn list_file(&self, prefix: &str) -> DFResult<FileMetaStream> {
        ...
        return self.tokio_runtime.lock().unwrap().block_on(async {
            let list_file_impl = || -> JniResult<FileMetaStream> {
                ...
                let fs = env.call_static_method_unchecked(
                    JavaClasses::get().cJniBridge.class.clone(),
                    JavaClasses::get().cJniBridge.method_get_hdfs_file_system.clone(),
                    JavaClasses::get().cJniBridge.method_get_hdfs_file_system_ret.clone(),
                    &[],
                )?.l()?;
                ...
                let file_status = env.call_method_unchecked(
                    fs,
                    JavaClasses::get().cHadoopFileSystem.method_get_file_status.clone(),
                    JavaClasses::get().cHadoopFileSystem.method_get_file_status_ret.clone(),
                ...
                let file_meta = FileMeta {
                    sized_file: SizedFile {
                        ...
                        size: env.call_method_unchecked(
                            file_status,
                            JavaClasses::get().cHadoopFileStatus.method_get_len.clone(),
                            JavaClasses::get().cHadoopFileStatus.method_get_len_ret.clone(),
                            &[],
                        )?.j()? as u64
                    },
                    last_modified: None
                ...
                impl Stream for HDFSSingleFileMetaStream {
                    ...
                    fn poll_next(self: Pin<&mut Self>, _cx: &mut Context<'_>) -> Poll<Option<DFResult<FileMeta>>> {
                ...
                return Ok(Box::pin(HDFSSingleFileMetaStream { file_meta, ended: false }));
            };
    ...
    async fn list_dir(&self, _prefix: &str, _delimiter: Option<String>) -> DFResult<ListEntryStream> {
        DFResult::Err(DataFusionError::NotImplemented("HDFSSingleFileObjectStore::list_dir not supported".to_owned()))
    }
    ...
    fn get_seekable_reader(&self) -> DFResult<Box<dyn ReadSeek + Send + Sync>> {
        return self.object_store.tokio_runtime.lock().unwrap().block_on(async {
            let reader_jni = || -> JniResult<Box<dyn ReadSeek + Send + Sync>>{
                ...
                let hdfs_input_stream = env.call_method_unchecked(
                    fs,
                ...
                let reader = Box::new(
                    HDFSFileReader::try_new(self.object_store.clone(), hdfs_input_stream, self.file.size)
                );
                Ok(reader)
            };
...
impl HDFSFileReader {
    pub fn try_new(object_store: HDFSSingleFileObjectStore, hdfs_input_stream: JObject<'static>, file_size: u64) -> HDFSFileReader {
        HDFSFileReader {
            object_store,
            hdfs_input_stream,
            file_size,
...
impl Read for HDFSFileReader {
    fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
        return self.object_store.tokio_runtime.lock().unwrap().block_on(async {
            ...
            let read_size = env.call_method_unchecked(
                self.hdfs_input_stream,
                JavaClasses::get().cHadoopFSDataInputStream.method_read.clone(),
                JavaClasses::get().cHadoopFSDataInputStream.method_read_ret.clone(),
                &[JValue::Object(buf.into())],
            )?.i()? as usize;

            self.pos += read_size as u64;
            Ok(read_size)
        }).map_err(|err: JniError| std::io::Error::new(std::io::ErrorKind::Other, err));
    }
}
        ...
        let seek_absolutely = |pos: u64| -> std::io::Result<u64> {
            return self.object_store.tokio_runtime.lock().unwrap().block_on(async {
                ...
                env.call_method_unchecked(
                    self.hdfs_input_stream,
                    JavaClasses::get().cHadoopFSDataInputStream.method_seek.clone(),
                    JavaClasses::get().cHadoopFSDataInputStream.method_seek_ret.clone(),
