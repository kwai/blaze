...
final class ShuffleInMemorySorter {
  ...
  /**
   * Whether to use radix sort for sorting in-memory partition ids. Radix sort is much faster but
   * requires additional memory to be reserved memory as pointers are added.
   */
  ...
  public ShuffleSorterIterator getSortedIterator() {
    ...
    if (useRadixSort) {
      ...
      MemoryBlock unused =
          new MemoryBlock(
              array.getBaseObject(), array.getBaseOffset() + pos * 8L, (array.size() - pos) * 8L);
