...
case class NativeWindowExec(
  ...
  private val nativeWindowExprs = windowExpression.map { named =>
  ...
  private val nativePartitionSpecExprs = partitionSpec.map { partition =>
    NativeConverters.convertExpr(partition)
  }

  private val nativeOrderSpecExprs = orderSpec.map { sortOrder =>
    pb.PhysicalExprNode
      .newBuilder()
      .setSort(
  ...
  override def doCanonicalize(): SparkPlan =
    WindowExec(windowExpression, partitionSpec, orderSpec, child).canonicalized
