...
abstract class BlazeColumnVector(val dataType: DataType) extends AutoCloseable {
  ...
  def getStruct(rowId: Int): BlazeColumnarStruct = {
    if (isNullAt(rowId)) return null
    new BlazeColumnarStruct(this, rowId)
  }

  def getArray(rowId: Int): BlazeColumnarArray

  def getMap(ordinal: Int): BlazeColumnarMap
  ...
  def getChild(ordinal: Int): BlazeColumnVector
}
