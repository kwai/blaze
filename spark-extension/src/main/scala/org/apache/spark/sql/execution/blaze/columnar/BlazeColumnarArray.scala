...
class BlazeColumnarArray(data: BlazeColumnVector, offset: Int, length: Int) extends ArrayData {
  ...
  override def getStruct(ordinal: Int, numFields: Int): BlazeColumnarStruct = {
    if (data == null) return null
    data.getStruct(offset + ordinal)
  }

  override def getArray(ordinal: Int): BlazeColumnarArray = {
    if (data == null) return null
  ...
  override def getMap(ordinal: Int): BlazeColumnarMap = {
    if (data == null) return null
