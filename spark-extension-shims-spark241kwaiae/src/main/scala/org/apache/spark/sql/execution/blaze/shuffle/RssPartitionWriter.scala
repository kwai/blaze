...
class RssPartitionWriter(
  ...
  private val realWriter = FieldUtils.readField(streamWriter, "realWriter", true)
    .asInstanceOf[StreamWriterWrapper[_, _]]
  private val emptyKey = FieldUtils.readField(streamWriter, "emptyKey", true)
    .asInstanceOf[UnifiedDataInputBuffer]
  private val partitionLengthMap = new Array[Long](numPartitions)

  tc.addTaskCompletionListener(_ => this.close())
  tc.addTaskFailureListener((_, _) => this.close())

  override def write(partitionId: Int, buffer: ByteBuffer): Unit = {
    val length = buffer.remaining()
    val valueBuffer = FieldUtils.readField(streamWriter, "value", true)
      .asInstanceOf[UnifiedDataInputBuffer]
    ...
    MethodUtils.invokeMethod(realWriter, true, "append",
      emptyKey,
      valueBuffer,
      Int.box(partitionId),
      Int.box(0))
  ...
  override def close(): Unit = realWriter.close()
