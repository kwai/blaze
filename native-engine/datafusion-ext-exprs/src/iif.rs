...
impl PartialEq<dyn Any> for IIfExpr {
    fn eq(&self, other: &dyn Any) -> bool {
        down_cast_any_ref(other)
            .downcast_ref::<Self>()
            .map(|x| self.condition_expr.eq(&x.condition_expr)
    ...
    fn evaluate(&self, batch: &RecordBatch) -> Result<ColumnarValue> {
        ...
        match condition {
            ...
            ColumnarValue::Scalar(_) => {
                self.falsy_expr.evaluate(batch)
            }
            ColumnarValue::Array(conditions) => {
                Ok(ColumnarValue::Array(compute_iif(
                    batch,
...
mod test {
    ...
    fn test() {
        ...
        let condition_expr = Arc::new(
            BinaryExpr::new(a.clone(), Gt, b.clone()));
        let if_expr = Arc::new(
            IIfExpr::new(condition_expr, a, b, DataType::Int32));

        let result = if_expr.evaluate(&batch).unwrap().into_array(6);
        let result: &Int32Array = as_primitive_array(&result);
        assert_eq!(result.into_iter().collect::<Vec<_>>(), vec![
            Some(5),
            Some(4),
            Some(3),
            Some(3),
