...
use crate::agg::agg_buf::{AggBuf, AggDynSet, AggDynStrSet};
use crate::agg::Agg;
use arrow::array::*;
use arrow::datatypes::*;
...
impl AggCollectSet {
    pub fn try_new(
        ...
        let field = Arc::new(Field::new("collect_set", arg_type.clone(), false));
        ...
        macro_rules! handle_fixed {
            ($ty:ident) => {{
                ...
                let w = agg_buf
                    .dyn_value_mut(addr)
                    .as_any_mut()
                    .downcast_mut::<AggDynSet<TNative>>()
                    .unwrap();
        ...
        macro_rules! handle_fixed_float {
            ($ty:ty) => {{
                ...
                let w = agg_buf
                    .dyn_value_mut(addr)
                    .as_any_mut()
                    .downcast_mut::<AggDynSet<[u8; size_of::<TNative>()]>>()
                    .unwrap();
                for v in value.iter().flatten() {
                    w.append(v.to_ne_bytes());
                }
            }};
        }

        match values[0].data_type() {
            DataType::Float32 => handle_fixed_float!(Float32),
            ...
            DataType::Timestamp(TimeUnit::Second, _) => handle_fixed!(TimestampSecond),
            DataType::Timestamp(TimeUnit::Millisecond, _) => handle_fixed!(TimestampMillisecond),
            DataType::Timestamp(TimeUnit::Microsecond, _) => handle_fixed!(TimestampMicrosecond),
            DataType::Timestamp(TimeUnit::Nanosecond, _) => handle_fixed!(TimestampNanosecond),
...
fn get_partial_updater(dt: &DataType) -> Result<fn(&mut AggBuf, u64, &ArrayRef, usize)> {
    ...
    match dt {
        DataType::Float32 => fn_fixed_float!(Float32),
        DataType::Float64 => fn_fixed_float!(Float64),
...
fn get_partial_buf_merger(dt: &DataType) -> Result<fn(&mut AggBuf, &mut AggBuf, u64)> {
    ...
    match dt {
        DataType::Float32 => fn_fixed!([u8; 4]),
        DataType::Float64 => fn_fixed!([u8; 8]),
