...
pub struct AggMinAccum {
    pub partial: ScalarValue,
}

impl AggAccum for AggMinAccum {
    fn as_any(&self) -> &dyn Any {
        self
    }
    ...
    fn partial_update_all(&mut self, values: &[ArrayRef]) -> Result<()> {
        macro_rules! handle {
            ($tyname:ident, $partial_value:expr) => {{
                type TArray = paste! {[<$tyname Array>]};
                let value = downcast_value!(values[0], TArray);
                if let Some(w) = arrow::compute::min(value) {
        ...
        match &mut self.partial {
            ...
            ScalarValue::Utf8(v) => {
                let value = downcast_value!(values[0], StringArray);
                if let Some(w) = arrow::compute::min_string(value) {
    ...
    fn partial_merge(&mut self, another: AggAccumRef) -> Result<()> {
        let another_min = another.into_any().downcast::<AggMinAccum>().unwrap();
        self.partial_merge_scalar(another_min.partial)
    }
