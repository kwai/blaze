...
use datafusion_ext_commons::{batch_size, df_execution_err, downcast_any, streams::coalesce_stream::CoalesceInput};
...
impl SortMergeJoinExec {
    ...
    fn create_join_params(&self, batch_size: usize) -> JoinParams {
        let on_left: Vec<usize> = self.on.iter().map(|on| downcast_any!(on.0, Column).unwrap().index()).collect();
        let on_right: Vec<usize> = self.on.iter().map(|on| downcast_any!(on.0, Column).unwrap().index()).collect();
