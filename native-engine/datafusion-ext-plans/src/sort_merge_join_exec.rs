...
use datafusion::{
    ...
    physical_plan::{
        joins::utils::{build_join_schema, check_join_is_valid, ColumnIndex, JoinFilter, JoinOn},
...
impl SortMergeJoinExec {
    ...
    fn create_join_params(&self, batch_size: usize) -> JoinParams {
        let on_left: Vec<usize> = self
            .on
            .iter()
            .map(|on| downcast_any!(on.0, Column).unwrap().index())
            .collect();
        let on_right: Vec<usize> = self
            .on
            .iter()
            .map(|on| downcast_any!(on.1, Column).unwrap().index())
            .collect();
