            ...
            let pcols = match P.mode {
                Semi | Anti => {
                    let probed_indices = probed_joined
                        ...
                        .collect::<Vec<_>>();
                    take_cols(probed_batch.columns(), probed_indices)?
                }
                Existence => {
                    let exists_col = Arc::new(BooleanArray::from(
                        probed_joined.into_iter().collect::<Vec<_>>(),
                    ));
                    [probed_batch.columns().to_vec(), vec![exists_col]].concat()
                }
            };
            ...
            let pcols = match P.mode {
                Semi | Anti => {
                    let map_indices = map_joined
                        ...
                        .map(|(idx, _)| idx as u32)
                        .collect::<Vec<_>>();
                    take_cols(self.map.data_batch().columns(), map_indices)?
                }
                Existence => {
                    let exists_col = Arc::new(BooleanArray::from(
                        map_joined.into_iter().collect::<Vec<_>>(),
                    ));
                    [self.map.data_batch().columns().to_vec(), vec![exists_col]].concat()
                }
            };
            self.as_mut().flush(pcols).await?;
        }
