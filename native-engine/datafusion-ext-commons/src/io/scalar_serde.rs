...
use crate::{
    ...
    io::{read_bytes_slice, read_len, write_len, read_u8, write_u8},
};
...
pub fn write_scalar<W: Write>(value: &ScalarValue, output: &mut W) -> Result<()> {
    ...
    match value {
        ...
        ScalarValue::TimestampSecond(Some(value), _) => write_primitive_valid_scalar(value.to_ne_bytes().as_slice(), output)?,
        ScalarValue::TimestampMillisecond(Some(value), _) => write_primitive_valid_scalar(value.to_ne_bytes().as_slice(), output)?,
        ScalarValue::TimestampMicrosecond(Some(value), _) => write_primitive_valid_scalar(value.to_ne_bytes().as_slice(), output)?,
        ScalarValue::TimestampNanosecond(Some(value), _) => write_primitive_valid_scalar(value.to_ne_bytes().as_slice(), output)?,
...
pub fn read_scalar<R: Read>(input: &mut R, data_type: &DataType) -> Result<ScalarValue> {
    ...
    Ok(match data_type {
        ...
        DataType::Timestamp(TimeUnit::Second, str) => ScalarValue::TimestampSecond(read_primitive_scalar!(input, 8, i64), str.clone()),
        DataType::Timestamp(TimeUnit::Millisecond, str) => ScalarValue::TimestampMillisecond(read_primitive_scalar!(input, 8, i64), str.clone()),
        DataType::Timestamp(TimeUnit::Microsecond, str) => ScalarValue::TimestampMicrosecond(read_primitive_scalar!(input, 8, i64), str.clone()),
        DataType::Timestamp(TimeUnit::Nanosecond, str) => ScalarValue::TimestampNanosecond(read_primitive_scalar!(input, 8, i64), str.clone()),
        DataType::Binary => {
